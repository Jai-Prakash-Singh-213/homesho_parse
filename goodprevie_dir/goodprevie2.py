...
def main():
    filename = "/home/desktop/goodprevie_dir/goodpreview%s.csv" %(time.strftime("%d%m%Y"))
    f = open(filename, "a+")
    ...
    for tds in  simpleinfo[::2]:
        ...
        if sitelink is not None:
            ...
            sql  = """select * from goodprevietable where redirectsite = "%s" limit 1""" %(sitelinktitle)
            cursor.execute(sql)
            ...
            for row in results:
                ...
                difrnce = str(difrnce)
                sql = """ update goodprevietable  set clicks="%s", newclick="%s",\
